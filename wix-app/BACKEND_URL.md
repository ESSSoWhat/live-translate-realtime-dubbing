...
2. Backend env `WIX_SYNC_SECRET` value matches the Wix Secrets Manager secret `LT_SYNC_SECRET` (name differs — Wix forbids names starting with `wix`; the values must match).
